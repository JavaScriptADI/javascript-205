
# JavaScript 205


# Programming Topics

1. Variables - Data types - operations - method
2. Data Structures - method - sort - find element
3. if / else / else if / switch - control flow
4. Loops - repeated operations
5. Functions - accept input - completes some instructions - gives result
6. class
7. DOM - Document Object Model
8. fetch - to get data from another source - RestAPI


## Homeworks
- [Homework 1](./homeworks/homework_1.md)
- [Homework 2](./homeworks/homework_2.md)
- [Homework 3](https://github.com/JavaScriptADI/javascript-205-homework-3.git)
- [Homework 4](https://github.com/JavaScriptADI/javascript-205-homework-4.git)
- [Homework 5](https://github.com/JavaScriptADI/javascript-205-homework-5.git)
- [Homework 6](https://github.com/JavaScriptADI/javascript-205-homework-6.git)
- [Homework 7](https://github.com/JavaScriptADI/javascript-205-homework-7.git)
- [Homework 8](https://github.com/JavaScriptADI/javascript-205-homework-8.git)

[Book](https://eloquentjavascript.net/)